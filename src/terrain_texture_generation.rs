...
use crate::utils::{Arr2d, ColorMapArray, HALF_PI, PI, TWO_POW_15_F32};
...
pub fn generate_terrain_texture(output: &mut ColorMapArray, heightmap: &Arr2d<f32>, buffer: &mut Arr2d<f32>,
     width: usize, ref_height: f32, shadow_direction: u8, sun_angle: f32, rng: &mut Mcg128Xsl64) {

    let mut noise: f32;

    add_environment_coloration(output, heightmap, buffer, width, ref_height, rng);
...
pub fn add_environment_coloration(output: &mut ColorMapArray, heightmap: &Arr2d<f32>, buffer: &mut Arr2d<f32>, width: usize, ref_height: f32, rng: &mut Mcg128Xsl64) {
    ...
    for x in 0..width {
        for y in 0..width {
            ...
            if let Some(pixel) = output.get_mut_pixel(y, x) {
                ...
                sum_of_heights = 0.0;
                n = 0.0;
                for mut dx in (-60..=60).step_by(30) {
                    for mut dy in (-60..=60).step_by(30) {

                        // essaye de convertir cette nouvelle position en indexes valides
                        x2_result = (dx + x as i32).try_into();  
                        y2_result = (dy + y as i32).try_into();

                        if let Ok(x2) = x2_result {  // si x est valide
                            if let Ok(y2) = y2_result {  // si y est valide
                                if let Some(h) = heightmap.get(x2, y2) {  // si la position est dans les limites du tableau
                                    sum_of_heights += (current_height - h).abs();  // additioner la différence
                                    n += 1.0
                                }
                            }
                        }
...
pub fn add_shadow(output: &mut ColorMapArray, heightmap: &Arr2d<f32>, width: usize, direction: u8, angle: f32, ref_height: f32) {
    ...
    let coef = ((angle  % HALF_PI).tan()).abs();

    let mut pixel;
    let mut shadowing: f32;
    ...
    for i in 0..width {
        for j in 0..width {
            ...
            pixel = output.get_mut_pixel(y, x).unwrap();
            local_height = *heightmap.get(x, y).unwrap(); 

            if local_height >= current_max_per_line[j] {
                current_max_per_line[j] = local_height;
                shadowing = 0.0;
            } else {
                shadowing = (local_height - current_max_per_line[j]).abs() / ref_height * 0.1 + 0.35;
            }
